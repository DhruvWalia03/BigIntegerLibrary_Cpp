...
int main(){
    ...
    cout << (b1<b2) << endl;
    cout << (b1>b2) << endl;
    cout << (b1.maximum(b2)).toString() << endl;
    cout << (b1.minimum(b2)).toString() << endl; */
...
BigInteger BigInteger::unaryminus() {
    setSign(!getSign());
    return (*this);
}
...
pair<string,long long> BigInteger::divide(string dividend, long long divisor) {
    ...
    quotient.resize(dividend.length());
